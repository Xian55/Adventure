...
| M3 | Inventory + pickups + HUD | Weapon/item pickups, hotbar/equip, Lua-driven HUD. | in progress — M3a: `src/items` (pure `Inventory`/`Item`/`Pickup`), destructibles drop item ids, coins/keys/potions collected + native-res HUD ✓. Left: Lua item defs, equip/hotbar, chests + keys |
