...
| M5 | Ranged + spells | Crossbow projectiles, data-driven spell system. | in progress — M5a: crossbow (`combat/Projectile`) reuses the melee charge/release (hold-draw, release-fire); bolts fly/stick to walls/hit enemies, charge scales speed+damage, per-weapon viewmodel. M5b: spells — `combat/Spell` Telekinesis force-push (mana + cone shove into hazards, R to cast, Lua defs) ✓. Left: more spells (flame/heal), cast VFX, rage-on-ranged |
