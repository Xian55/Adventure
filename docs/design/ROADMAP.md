...
| **M2** | Combat slice (**target**) | Melee swing state machine + sweeping hitbox, kick knockback, shield block, one skeleton (approach/attack/stagger/die), arena script, `trigger_hurt` hazard kill. **Nail the feel.** | in progress — M2a directional melee + hitbox + sword anim ✓; M2b skeleton (approach/stagger/die) + hit resolution ✓; M2c kick ✓, shield block + enemy strike-back + player health/respawn ✓; M2d rage→berserk meter ✓; billboard enemy sprites behind the `RenderKind` seam ✓. Left: arena/`trigger_hurt` script, real sprite art |
