...
| M3 | Inventory + pickups + HUD | Weapon/item pickups, hotbar/equip, Lua-driven HUD. | in progress — M3a: `src/items` (pure `Inventory`/`Item`/`Pickup`), destructibles drop item ids, coins/keys/potions collected + native-res HUD ✓; M3b: chests + keys (`Container`, E to open, locked needs a key, loot spills; props/chests solid + standable via `Collide`) ✓. Left: Lua item defs, equip/hotbar |
