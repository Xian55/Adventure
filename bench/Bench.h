#pragma once
// Tiny header-only micro-benchmark harness. Single responsibility: run named cases, time them,
// and fail (nonzero exit) if any exceeds its ms budget — so it works as an automated perf gate
// in CTest/CI. Not a statistics package; it reports total ms, us/iteration, and pass/fail vs budget.
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <functional>
...
namespace advbench
{
		...
		public:
			...
			int runAll()
			{
				...
				std::printf("%-30s %10s %10s %10s %10s   %s\n",
					"case", "iters", "total ms", "us/it", "budget ms", "result");
				std::printf("--------------------------------------------------------------------------------\n");
				...
				for (auto& c : m_cases)
				{
					...
					for (long i = 0; i < warm; ++i) c.body();

					const auto t0 = clock::now();
					for (long i = 0; i < c.iterations; ++i) c.body();
					const double ms =
					...
					std::printf("%-30s %10ld %10.2f %10.3f %10.2f   %s\n",
						c.name.c_str(), c.iterations, ms, usPerIt, c.budgetMs, ok ? "OK" : "OVER BUDGET");
				}
	...
	struct AutoReg { explicit AutoReg(Case c) { Registry::get().add(std::move(c)); } };
