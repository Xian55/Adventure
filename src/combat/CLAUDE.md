...
| `Loadout.{h,cpp}` | Equip / weapon-swap. `weaponDefFor(itemId, swordDef)` (Sword uses the Lua-tuned def; Dagger = fast/short/light, Mace = slow/heavy/knockback), `nextWeapon(inv, current)` (cycle the weapons the player holds), `isWeapon`/`weaponName`. Pure/tested. `main` binds the cycle to `Action::NextWeapon` and re-derives the active `WeaponDef`. |
