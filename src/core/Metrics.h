...
// Performance data collector. Single responsibility: measure and aggregate — it does NOT draw
// (see render/MetricsOverlay) and does NOT touch the OS directly (see core/Platform). No raylib,
// so it is unit-testable headlessly. The clock is injectable for deterministic tests.
//
// Usage in the game loop:
//   metrics.beginFrame();
//   { Metrics::Scope s(metrics, "scene"); ...work... }
//   metrics.setLuaBytes(...);
//   metrics.endFrame();
namespace adventure
{
	class Metrics
	{
		public:
			...
			struct Scope
			{
				Metrics& m; const char* n;
				Scope(Metrics& mm, const char* name) : m(mm), n(name) { m.begin(n); }
				~Scope() { m.end(n); }
			};
		...
		private:
			struct Section { const char* name; double startT; double accum; float smoothMs; };
