...
| `Viewmodel.{h,cpp}` | First-person torch (left) + **equipped weapon** (right — `weapon` param: sword / dagger / mace geometry) + a **boot kick** (thrusts up from bottom-centre on F, `kick` param 1→0), drawn over the world with depth-test off (own near camera, `rlgl`) inside the low-res RT. Procedural placeholder geometry + torch flicker + movement bob, until sprites/models exist. |
