...
namespace adventure
{
	namespace
	{
		float mib(std::size_t bytes) { return (float)bytes / (1024.0f * 1024.0f); }
	}
	...
	void drawMetricsOverlay(const Metrics& m, bool visible)
	{
		...
		DrawText(TextFormat("FPS %3.0f   %5.2f ms  (max %5.2f)", m.fps(), m.frameMs(), m.frameMsMax()),
			x, cy, font, m.frameMsMax() > 20.0f ? warn : head);
		cy += line;

		DrawText(TextFormat("CPU %4.1f%%   RSS %6.1f MB   Lua %5.1f KB",
			m.cpuPercent(), mib(m.rssBytes()), (float)m.luaBytes() / 1024.0f), x, cy, font, val);
		cy += line;
